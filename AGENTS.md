...
This repo ships project-level [oh-my-claudecode (OMC)](https://github.com/Yeachan-Heo/oh-my-claudecode) config under `.claude/`. Install the plugin **once per machine** in Claude Code so the routing, agents, and `/oh-my-claudecode:*` commands are available:
...
Then run `/oh-my-claudecode:omc-setup` to finish wiring (HUD statusline, hooks, defaults). Without the plugin, `.claude/CLAUDE.md` and the `omc-reference` skill still load, but OMC commands and agents will not work.
