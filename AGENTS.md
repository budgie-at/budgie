...
45. **Magic strings that name a thing become an enum.** Subsystem names, error sources, telemetry channels, and storage keys that are referenced by ≥2 sites — define an enum (rule 28) and use it everywhere. `'chat' | 'embedding' | 'stt'` literal unions and string returns from `getSomeKind()` are red flags.
