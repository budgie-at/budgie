...
- **Worklet safety**: `@rnw-community/shared` guards are required across this package exactly like everywhere else in the repo. The ONLY restriction is worklet bodies: no non-worklet function of any kind may be called inside `useAnimatedStyle`/`useAnimatedProps`/`useAnimatedScrollHandler`. Compute plain values in the hook/component body (using the guards there) and capture them into the worklet. This is not an instruction to avoid `@rnw-community/shared` — plain `!== null` / `typeof` checks must never replace its guards outside worklets.
