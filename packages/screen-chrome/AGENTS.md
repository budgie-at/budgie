...
- **`ScreenChromeScrollView` has no keyboard-aware auto-scroll.** Unlike `FormPage`'s `KeyboardAwareScrollView` (from `react-native-keyboard-controller`), `ScreenChromeScrollView` is a plain `Animated.ScrollView` — swapping it for a keyboard-aware scroller would drop the reanimated `scrollY` that drives the header collapse and blur backdrop. A `CollapsibleChromePage` `footer` still rises above the keyboard via `KeyboardStickyView` (see `StickyFooterBand`), so the submit CTA stays reachable; there is just no automatic scroll-into-view for a focused input near the bottom of tall forms. Accepted for the account create/edit forms (short enough that the focused field stays above the keyboard); reassess before putting a long, many-field form on `CollapsibleChromePage`.
