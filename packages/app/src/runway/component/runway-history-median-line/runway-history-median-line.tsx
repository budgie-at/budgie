...
export const RunwayHistoryMedianLine = ({ value, maxValue }: Props) => {
    ...
    const labelY = medianY - LABEL_GAP;
    const backdropY = labelY - LABEL_FONT_SIZE;
    const backdropWidth = label.length * LABEL_CHAR_WIDTH + LABEL_PADDING * 2;
    const backdropHeight = LABEL_FONT_SIZE + LABEL_PADDING;

    return (
        <>
            <Rect x={0} y={backdropY} width={backdropWidth} height={backdropHeight} rx={BACKDROP_RADIUS} fill={colors.markerBackground} />
            <Line
                x1={0}
                y1={medianY}
                ...
                strokeWidth={1}
            />
            <SvgText x={LABEL_PADDING} y={labelY} fill={colors.label} fontSize={LABEL_FONT_SIZE} fontWeight="600">
                {label}
            </SvgText>
        </>
    );
