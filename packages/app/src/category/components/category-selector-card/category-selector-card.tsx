...
export const CategorySelectorCard = (props: Props) => {
    const { className, isSelected, title, onSelect, id, icon, testID } = props;

    const handleSelect = () => void onSelect(id);

    const iconVariant = isSelected ? 'primary' : 'ghost';

    return (
        <HapticPressable className={cn(cardVariants({ isSelected }), className)} onPress={handleSelect} testID={testID}>
