...
class ConsolidationCoordinatorService {
    ...
    async consolidate(onProgress?: (processedCandidateGroupCount: number) => void): Promise<ConsolidationResultInterface> {
        ...
        return runners.reduce(async (resultPromise, runner) => {
            ...
            const result = await runner.process(claimedTransactionIds, publishProgress);
    ...
    async countAutoCandidates(): Promise<number> {
        const claimedTransactionIds = new Set<number>();
        const runners = consolidationRuleRegistryService.getRunners();
        const count = 0;
        ...
        return runners.reduce(async (countPromise, runner) => {
            ...
            const runnerCount = await runner.countCandidates(claimedTransactionIds);
