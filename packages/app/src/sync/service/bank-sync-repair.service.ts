...
hidden_sources AS (SELECT source.id AS transaction_id, source.operated_at, date(source.operated_at, 'unixepoch') AS operated_day, source.type AS transaction_type, source.title, source.external_id, moved.account_id, moved.type AS entry_type, moved.amount, COALESCE(moved.mcc_category_id, -1) AS mcc_category_id FROM transactions source INNER JOIN transaction_entries moved ON moved.original_transaction_id = source.id AND moved.deleted_at IS NULL WHERE source.external_source = 'PRIVATBANK' AND source.deleted_at IS NULL AND source.consolidation_parent_transaction_id IS NOT NULL),
...
`;
    ...
    private async removeDuplicatesInner(): Promise<BankSyncDuplicateRepairResultInterface> {
        return transactionAsync(db, tx => this.removeDuplicatesInTransaction(tx));
    }
    ...
    private async removeDuplicatesInTransaction(tx: DB): Promise<BankSyncDuplicateRepairResultInterface> {
        ...
        return this.buildRepairResult(candidates, updatedTransactionIds, repairedEntryCount);
    }
