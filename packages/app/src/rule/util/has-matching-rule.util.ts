...
const doesRuleApplySameOutcome = (
    rule: RuleWithRelationsEntityInterface,
    ...
    const tagsMatch =
        ruleTagIds.length === suggestRuleData.tagIds.length &&
        suggestRuleData.tagIds.every(tagId => ruleTagIds.includes(tagId));
...
export const hasMatchingRule = (
    ...
    rules.some(
        rule => doesRuleMatchTransaction(rule, transactionInput) && doesRuleApplySameOutcome(rule, suggestRuleData)
    );
