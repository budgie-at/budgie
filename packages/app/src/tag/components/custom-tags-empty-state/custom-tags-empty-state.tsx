...
export const CustomTagsEmptyState = ({ search }: Props) => {
    const { t } = useLingui();

    const icon = isNotEmptyString(search) ? 'Search' : 'Tag';
    const title = isNotEmptyString(search) ? t`No Results` : t`No Tags Yet`;
    const description = isNotEmptyString(search) ? t`No tags match your search` : t`Create tags to organize your transactions`;
