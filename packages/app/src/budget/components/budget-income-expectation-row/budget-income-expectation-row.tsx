...
export const BudgetIncomeExpectationRow = ({ category, expectedAmount, onEdit, onRemove }: Props) => {
    const { t } = useLingui();
    ...
    const formattedExpectedAmount = formatMoney(expectedAmount, defaultInstrument.symbol);

    const categoryTitle = category.title;
    const confirmDescription = t`This will remove the expected income for "${categoryTitle}". The category itself will not be deleted.`;
    ...
    return (
        <>
                ...
                <HorizontalCell
                    ...
                    right={
                        <View className="ml-auto flex-row items-center gap-x-sm">
                            <Text className="text-secondary-foreground font-medium">{formattedExpectedAmount}</Text>
            ...
            <ConfirmActionBottomSheet
                ...
                title={t`Remove Expected Income`}
            />
        </>
    );
};
