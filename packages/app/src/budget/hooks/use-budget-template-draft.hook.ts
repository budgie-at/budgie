...
import { resolveGenericBudgetTemplate } from '../utils/resolve-generic-budget-template.util';
...
export const useBudgetTemplateDraft = (kind: BudgetTemplateKindEnum | null): BudgetTemplateResolutionInterface => {
    ...
    if (kind === BudgetTemplateKindEnum.GENERIC) {
        const currencyCode = instrument?.code ?? '';
        ...
        const draft = resolveGenericBudgetTemplate(categoriesData, currencyCode);
