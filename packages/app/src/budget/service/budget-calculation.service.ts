...
class BudgetCalculationService {
    ...
    private calculateOverallStats(
        ...
        const totalSpent = this.calculateTotalSpent(expenseTransactions);
        const overallRemaining = budget.overallLimit - totalSpent;
        const overallPercentage = budget.overallLimit > 0 ? (totalSpent / budget.overallLimit) * 100 : 0;
        const overallStatus = this.getStatus(overallPercentage);

        return { totalSpent, overallRemaining, overallPercentage, overallStatus };
    }
    ...
    private calculatePaceStats(budget: BudgetEntityInterface, totalSpent: number): PaceStatsInterface {
        ...
        const dailyBudget = daysInPeriod > 0 ? budget.overallLimit / daysInPeriod : 0;
    ...
    private calculateCategoryStatuses(
        ...
        return categoryLimits.map(categoryLimit => {
            const spent = this.calculateAmountForCategory(categoryLimit.categoryId, expenseTransactions);
            const remaining = categoryLimit.limit - spent;
            const percentage = categoryLimit.limit > 0 ? (spent / categoryLimit.limit) * 100 : 0;
    ...
    private calculateIncomeStatuses(
        ...
        return incomeExpectations.map(incomeExpectation => {
            const actual = this.calculateAmountForCategory(incomeExpectation.categoryId, incomeTransactions);
            const variance = actual - incomeExpectation.expectedAmount;

            return {
                categoryId: incomeExpectation.categoryId,
                categoryName: incomeExpectation.category.title,
                expected: incomeExpectation.expectedAmount,
                actual,
                variance
            };
        });
