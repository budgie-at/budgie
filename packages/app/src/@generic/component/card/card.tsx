...
export const Card = ({ className, onPress, variant = 'primary', size = 'lg', testID, ...rest }: PropsWithChildren<Props>) => {
    const Component = isDefined(onPress) ? HapticPressable : View;

    return <Component testID={testID} className={cn(cardVariants({ size, variant }), className)} onPress={onPress} {...rest} />;
};
