...
export const ImportColumnMapField = ({ control, name, label, headers, isRequired = false }: Props) => {
    const renderField = ({ field: { value, onChange }, fieldState: { error } }: UseControllerReturn<ImportColumnMapFormValues>) => (
        <View className="gap-y-xs">
            ...
            <ImportColumnMapper value={value} headers={headers} onSelect={onChange} hasError={isNotEmptyString(error)} />
