import { AccountBalanceEntityInterface, transactionAsync } from '@budgie/contracts';
...
class AccountBalanceIncrementalService {
    async updateAllBalances(truncate: boolean, tx?: DB): Promise<void> {
        if (isDefined(tx)) {
            await this.updateAllBalancesInner(truncate, tx);

            return;
        }

        await transactionAsync(db, async innerTx => this.updateAllBalancesInner(truncate, innerTx));
    }
    ...
    private async updateAllBalancesInner(truncate: boolean, tx: DB): Promise<void> {
