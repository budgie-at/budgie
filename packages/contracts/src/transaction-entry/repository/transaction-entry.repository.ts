...
export class TransactionEntryRepository {
    ...
    async bulkCreate(inputs: TransactionEntryCreateEntityInterface[], tx?: Transaction): Promise<TransactionEntryEntityInterface[]> {
    ...
    async create(input: TransactionEntryCreateEntityInterface, tx?: Transaction): Promise<TransactionEntryEntityInterface> {
        const [transactionEntry] = await (tx ?? this.db).insert(TransactionEntryEntityTable).values([input]).returning();

        return transactionEntry;
    }

    async deleteByTransactionId(transactionId: number, tx?: Transaction): Promise<void> {
