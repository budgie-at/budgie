...
export class TransactionRepository extends BaseTransactionFilterRepository {
    ...
    async convertTransfersFromAccountToIncome(accountId: number, tx?: DB, additionalFilter?: SQL): Promise<void> {
        await (tx ?? this.db)
            .update(TransactionEntityTable)
    ...
    async convertTransfersToAccountToExpense(accountId: number, tx?: DB, additionalFilter?: SQL): Promise<void> {
        await (tx ?? this.db)
            .update(TransactionEntityTable)
    ...
    async unconsolidateTransfersByAccountIds(accountIds: number[], tx?: DB): Promise<void> {
