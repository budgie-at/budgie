...
export const EXISTING_TRANSFER_BRIDGE_CANDIDATES_SQL = `
            ...
            FROM (
                ...
                INNER JOIN accounts bridge_account ON
                    ...
                    AND bridge_account.instrument_id != source_account.instrument_id
                INNER JOIN transactions existing_transfer ON
                    existing_transfer.type = '${TransactionTypeEnum.TRANSFER}'
                    AND existing_transfer.deleted_at IS NULL
                    AND existing_transfer.consolidation_parent_transaction_id IS NULL
                    AND existing_transfer.from_account_id = bridge_account.id
                    AND ABS(existing_transfer.operated_at - bridge_income_tx.operated_at) <= ${TRANSFER_PAIR_FAST_TIME_WINDOW_SECONDS}
                INNER JOIN transaction_entries existing_bridge_entry ON
                    ...
                    AND existing_bridge_entry.amount = bridge_income_entry.amount
                INNER JOIN transaction_entries existing_target_entry ON
                    ...
                    AND existing_target_entry.account_id != bridge_account.id
                    AND existing_target_entry.amount = bridge_income_entry.amount
