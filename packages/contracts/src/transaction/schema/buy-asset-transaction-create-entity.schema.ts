...
export const BuyAssetTransactionCreateEntitySchema = BaseTransferTransactionCreateEntitySchema.superRefine(
    ({ entries, exchangeRate, toAccountId, fromAccountId }, context) => {
        ...
        const feeEntry = entries.find(({ accountId }) => accountId !== fromAccountId && accountId !== toAccountId);
        ...
        if (toEntry.type !== TransactionEntryTypeEnum.DEBIT) {
            context.addIssue({
                ...
                message: "to-entry must be 'debit' (asset amount enters the to account)"
            });
        ...
        if (isDefined(feeEntry)) {
            const feeEntryIndex = entries.findIndex(({ accountId }) => accountId === feeEntry.accountId);

            if (feeEntry.type !== TransactionEntryTypeEnum.DEBIT) {
                context.addIssue({
                    code: 'custom',
        ...
        const convertToFromMicroUnits = (amountMicroUnits: number, isToAccountLeg: boolean): number => {
            if (!isToAccountLeg) {
