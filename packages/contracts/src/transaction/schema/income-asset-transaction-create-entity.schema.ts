...
export const IncomeAssetTransactionCreateEntitySchema = convertToCreateEntitySchema(IncomeAssetTransactionEntitySchema)
    ...
    .superRefine(({entries}, context) => {
        ...
        if (entry.amount <= 0) {
            context.addIssue({ code: 'custom', path: ['entries'], message: 'income asset entry amount must be > 0' });
        }
