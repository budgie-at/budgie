    ...
    .superRefine(({ entries, exchangeRate }, context) => {
        ...
        if (entryA.accountId === entryB.accountId) {
            context.addIssue({
                code: 'custom',
                path: ['entries'],
                message: 'sell asset transaction entries must have different accounts'
            });
        ...
        if (entryA.instrumentId === entryB.instrumentId) {
            context.addIssue({
                code: 'custom',
                path: ['entries'],
                message: 'sell asset transaction entries must have different instruments'
            });
        ...
        if (exchangeRate === 1) {
            context.addIssue({
                code: 'custom',
                path: ['exchangeRate'],
                message: 'sell asset transaction exchange rate must be > 1'
            });
        }
    });
