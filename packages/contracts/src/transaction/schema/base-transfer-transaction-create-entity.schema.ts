...
export const BaseTransferTransactionCreateEntitySchema = TransactionCreateEntitySchema.omit({ [TransactionAssociationEnum.ENTRIES]: true }).extend({
    [TransactionAssociationEnum.ENTRIES]: array(TransactionEntryCreateEntitySchema.omit({ transactionId: true })).min(2).max(3)
}).superRefine(({ fromAccountId, toAccountId, entries, type }, context) => {
