...
const OgImage = async ({ params }: { params: Promise<{ lang: string }> }) => {
    ...
    return createFeatureOgImage(t(i18n)`Convert Income to Refund`, t(i18n)`Link refund income back to the expense it reverses — full or partial, automatic or manual, always reversible.`, [
        t(i18n)`refunds`,
        t(i18n)`analytics`,
        t(i18n)`cleanup`
    ]);
