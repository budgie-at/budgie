...
class ErsteExternalIdGenerator {
    @Log(
        (row, iban) => `enter iban=${iban} date=${row.date.toISOString()} amount=${row.amount} reference="${row.reference}"`,
        (result, row, iban) => `done iban=${iban} date=${row.date.toISOString()} amount=${row.amount} externalId=${result}`,
...
export const ersteExternalIdGenerator = new ErsteExternalIdGenerator();
