...
class BinanceMapper {
    ...
    mapBalanceToAccount(asset: string, wallet: BinanceWalletEnum, balance: number): BankAccountInterface {
        return {
            ...
            balance,
            creditLimit: 0,
            type: BankAccountTypeEnum.CRYPTO,
    ...
    mapDepositToTransaction(deposit: BinanceDepositApiInterface, accountId: string): BankTransactionInterface | null {
    ...
    mapWithdrawalToTransaction(withdrawal: BinanceWithdrawalApiInterface, accountId: string): BankTransactionInterface | null {
    ...
    mapFiatOrderToTransaction(order: BinanceFiatOrderApiInterface, accountId: string, isDeposit: boolean): BankTransactionInterface | null {
        ...
        return {
            ...
            type: isDeposit ? BankTransactionTypeEnum.INCOME : BankTransactionTypeEnum.EXPENSE,
            description,
            amount,
            operationAmount: amount - feeAmount,
            feeAmount
    ...
    mapC2cOrderToTransaction(order: BinanceC2cOrderApiInterface, accountId: string): BankTransactionInterface | null {
        const amount = this.parseBinanceAmount(order.amount);
        if (!isDefined(amount)) {
            return null;
        }
        ...
        return {
            ...
            type: isBuy ? BankTransactionTypeEnum.INCOME : BankTransactionTypeEnum.EXPENSE,
            description,
            amount,
            operationAmount: amount,
            feeAmount: 0
    ...
    mapEarnRewardsToTransactions(asset: string, accountId: string, rewards: BinanceEarnRewardApiInterface[]): BankTransactionInterface[] {
    ...
    private mapEarnMonthBucket(asset: string, accountId: string, bucket: BinanceEarnMonthBucket): BankTransactionInterface | null {
