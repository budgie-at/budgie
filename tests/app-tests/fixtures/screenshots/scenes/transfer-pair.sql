-- Scene overlay: transfer pairs, one still loose and one already consolidated.
--
-- Two different screens need two different states:
--
--   * `convert-to-transfer` / `transfer-pair-detection-1` need an UNconsolidated
--     pair — an expense on one account and a matching income on another, same
--     amount, hours apart, `consolidation_type` and
--     `consolidation_parent_transaction_id` both NULL — so the long-press menu
--     can offer the conversion.
--   * `budgie://consolidation-source` shows the SOURCES of an ALREADY
--     consolidated transaction, so it needs a canonical TRANSFER carrying
--     `consolidation_type = 'TRANSFER_PAIR'` with its two source transactions
--     pointing at it through `consolidation_parent_transaction_id`.
--
-- The canonical transfer mirrors what `ConsolidationMutationService` writes: a
-- TRANSFER with a CREDIT entry on the source account and a DEBIT entry on the
-- destination, both with `category_id = NULL`.
--
-- Owns transactions 2100-2199.
...
-- The loose pair. Same amount, 3 hours apart, different accounts.
...
-- The already-consolidated pair: canonical transfer 2110 with sources 2111/2112.
