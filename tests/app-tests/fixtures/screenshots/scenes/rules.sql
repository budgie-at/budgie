-- Scene overlay: the categorisation rules list.
--
-- A rule is `rules` (enabled + condition_match_type) plus its `rule_conditions`
-- and `rule_actions`. The table has no name column, so the list row is rendered
-- from the conditions and action pills, which is why every rule here carries
-- readable, locale-appropriate condition values.
--
-- Rules 4 and 5 are the deliberate conflict: both match the same supermarket
-- title and both SET_CATEGORY, to different categories. Rule 5 is disabled, so
-- the list shows the disabled state as well.
--
-- Schema CHECK constraints require `category_id` for SET_CATEGORY, `tag_id` for
-- ADD_TAG, `account_id` for CONVERT_TO_TRANSFER and a `secondary_value` for
-- BETWEEN, all of which are honoured below.
--
-- Owns rules 1-5, rule_conditions 1-9 and rule_actions 1-7.
...
-- Condition values that name a merchant have to read like that locale's real
-- statement text, so they are branched exactly like the locale overlays.
