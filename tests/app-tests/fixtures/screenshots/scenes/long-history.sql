-- Scene overlay: 18 months of history instead of 45 days.
--
-- The base dataset only spans 45 days, so "This Year" and "All Time" render the
-- same list as "This Month", the monthly-trend analytics have a single bar and
-- the recurring detector has nothing to chew on. This overlay replays the
-- newest 40 rows of the base ledger once per month for the previous 17 months,
-- with a deterministic per-month drift so the trend line is not a flat bar
-- chart.
--
-- Replaying the base rows (rather than inventing new merchants) keeps every
-- locale correct for free: the titles are already the ones the locale overlay
-- wrote, and the amounts are already at that locale's scale.
--
-- Only months in the PAST are generated, so `verify_database`'s
-- "no transaction after the capture day" assertion still holds.
--
-- Owns transactions 3000-6999.
...
-- A repeatable +-12% drift keyed on the month and the row, so the monthly trend
-- has shape without any month drifting far enough to break the recurring
-- detector's `max_amount <= min_amount * 2` rule.
