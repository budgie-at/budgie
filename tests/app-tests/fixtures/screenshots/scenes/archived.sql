-- Scene overlay: archived and inactive accounts.
--
-- `accountRepository.getAllArchived()` selects top-level accounts with
-- `deleted_at IS NOT NULL`; `getAllInactive()` selects live top-level accounts
-- with `is_active = 0`. They are two different screens
-- (`budgie://settings/archived`, `budgie://settings/inactive`), so this overlay
-- seeds both: 2 archived accounts and 1 inactive one, each with a balance
-- snapshot and a short history so the cards are not empty.
--
-- Owns accounts 16-18, account_balances 116-118 and transactions 2200-2299.
...
SELECT
    ...
    0,
    -- `getNetWorth` filters on deleted_at but NOT on is_active, so an inactive
    -- account left in net worth would inflate the home header while showing in
    -- no section.
...
-- A short ledger on each account, so the restore preview is not an empty card.
...
-- The archived cards still show a balance, so the snapshot has to absorb the
-- ledger above exactly like showcase.sql's opening snapshot does.
