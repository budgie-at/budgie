-- Scene overlay: debt you lent and debt you borrowed, one partly settled.
--
-- Home renders two debt sections driven by `accounts.type = 'DEBT'` plus
-- `debt_type`: `LENT` is "owed to you", `BORROW` is "you owe". `debt_events`
-- carries the OPEN/CLOSE ledger the debt account detail screen lists, so a
-- partial settlement is one OPEN event and a smaller CLOSE event against the
-- same account.
--
-- Account 7 is the lent debt the storyboard deep-links as
-- `budgie://account/7/details`.
--
-- Account ids are pinned: 7 is the lent debt (`budgie://account/7/details`)
-- and 11 the borrowed one, because the landing capture config addresses them
-- by id.
--
-- Owns accounts 7 and 11, account_balances 107 and 111, transactions 1200-1299
-- and debt_events 1-9.
...
-- The ledger. 1200/1201 open the two debts, 1202 is the partial repayment
-- Daniel already made, which is what turns account 7 into a partly settled
-- debt instead of an untouched one.
...
-- Debt balances come from the ledger alone, so the snapshot is zero-based.
