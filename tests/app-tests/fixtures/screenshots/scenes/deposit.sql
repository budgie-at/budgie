-- Scene overlay: a term deposit with a maturity date and accrued interest.
--
-- `accounts.type = 'DEPOSIT'` puts the account in the Deposit section on Home;
-- `accounts.deadline` is the maturity date and `accounts.interest_rate` the
-- annual rate the detail screen prints. `target_balance` is the amount the
-- deposit matures at, so the progress toward maturity is real.
--
-- The accrued interest is a ledger of monthly INCOME transactions into the
-- deposit account rather than a computed figure, so the detail screen's history
-- is populated too.
--
-- Account id 8 is pinned, because the landing capture config deep-links the
-- deposit as `budgie://account/8/details`.
--
-- Owns account 8, account_balances 108 and transactions 1300-1399.
...
-- Eight monthly interest credits, newest 4 days ago. `-N months` keeps the day
-- of month stable across month lengths, exactly like shift-dates.sql relies on.
...
-- The principal is the opening snapshot; the interest ledger above lands on
-- top of it, so the card shows principal plus everything accrued so far.
