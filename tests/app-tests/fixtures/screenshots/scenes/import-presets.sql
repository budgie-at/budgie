-- Scene overlay: a bank whose CSV statements have already been imported once.
--
-- IMPORTANT: the CSV column-mapping presets themselves are NOT database rows.
-- They are app constants (`packages/app/src/import/constant/import-presets.constant.ts`,
-- keyed by `ImportPresetEnum`), so the preset chips render on the import screen
-- whatever the database holds. What this overlay CAN seed, and what the import
-- scenes actually lack, is the surrounding state of a bank that has been
-- imported from before:
--
--   * a file-import `bank_integrations` row (`provider = 'CSV'`, `token = ''`,
--     which is the shape the partial unique index reserves for file imports),
--   * an account linked to it through `integration_id` + `external_source`,
--   * previously imported transactions carrying `external_id` and
--     `external_source = 'CSV'`, so a re-import demonstrably de-duplicates.
--
-- Owns bank_integrations 2, account 19, account_balances 119 and transactions
-- 2400-2499.
